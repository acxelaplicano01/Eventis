<div>
    ...
    <div class="dark:bg-gray-900">
        <div class="">
            <div class="bg-white overflow-hidden border sm:rounded-lg px-4 py-4 dark:bg-gray-800 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white">
                ...
                <!-- Modal de Crear -->
                @if ($isOpen)
                    @include('livewire.Patrocinador.create') <!-- Asegúrate que esta ruta sea correcta -->
                @endif
                        ...
                        <tbody>
                            @foreach($patrocinadores as $patrocinador)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-600 bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    <td class="px-6 py-4 dark:text-white">{{ $patrocinador->id }}</td>
                                    <td class="px-6 py-4">
                                        @if($patrocinador->Foto)
                                            <img src="{{ asset('storage/' . $patrocinador->Foto) }}"
                                                alt="Foto" class="w-12 h-12 object-cover">
                                        @else
                                            Sin Foto
                                        @endif
                                    </td>
                                    <td class="px-6 py-4">{{ $patrocinador->descripcion }}</td>
                                    <td class="px-6 py-4">
                                        <button wire:click="edit({{ $patrocinador->id }})"
                                        ...
                                        <button wire:click="confirmDelete({{ $patrocinador->id }})"
...
<div class="fixed z-50 inset-0 flex items-center justify-center overflow-y-auto ease-out duration-400">
    ...
    <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"
        ...
        <div class="p-6">
            ...
            <p>¿Estás seguro de que deseas eliminar al patrocinador:  Esta
